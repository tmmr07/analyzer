...
#' @return 作成した重回帰モデルを格納したリスト

make_mrm <- function(data_simulation, old_features, new_features, all_features, agent_type, output_path) {

	if (agent_type == "distributed") {
		write_lines("----------------------分散救助エージェントの重回帰モデルを作成----------------------", output_path, append = TRUE)
	} else if (agent_type == "concentration") {
		write_lines("----------------------集中救助エージェントの重回帰モデルを作成----------------------", output_path, append = TRUE)
	}
	...
	write_lines("旧指標群を説明変数とした重回帰モデルの構築", output_path, append = TRUE)
	...
	model_list_old <- purrr::map(response_variable_names, ~{
		...
		# 欠損値を除外して，重回帰モデルを構築
		old_model <- lm(old_model_formula, data = model_data_old, na.action = na.omit)
	...
	write_lines("", output_path, append = TRUE)
	write_lines("", output_path, append = TRUE)
	write_lines("新指標群を説明変数とした重回帰モデルの構築", output_path, append = TRUE)
	...
	model_list_new <- purrr::map(response_variable_names, ~{
	  ...
	  # 欠損値を除外して，重回帰モデルを構築
	  new_model <- lm(new_model_formula, data = model_data_new, na.action = na.omit)
	...
	write_lines("", output_path, append = TRUE)
	write_lines("", output_path, append = TRUE)
	write_lines("総合指標群を説明変数とした重回帰モデルの構築", output_path, append = TRUE)
	...
	model_list_all <- purrr::map(response_variable_names, ~{
		...
		# 欠損値を除外して，重回帰モデルを構築
		all_model <- lm(all_model_formula, data = model_data_all, na.action = na.omit)
	...
	if (agent_type == "distributed") {
		write_lines("----------------------分散救助エージェントの重回帰モデルの作成終了----------------------", output_path, append = TRUE)
		write_lines("", output_path, append = TRUE)
	} else if (agent_type == "concentration") {
		write_lines("----------------------集中救助エージェントの重回帰モデルの作成終了----------------------", output_path, append = TRUE)
		write_lines("", output_path, append = TRUE)
	}
