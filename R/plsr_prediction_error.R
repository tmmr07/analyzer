...
#' @param plsr_models 作成したPLSRモデルのリスト
#'
#' @return 計算した予測誤差

calculate_plsr_errors <- function(data_simulation, old_features, new_features, all_features, agent_type, output_path, plsr_models) {
  ...
  if (agent_type == "distributed") {
    write_lines("----------------------分散救助エージェントのPLSRモデルにおける予測誤差を算出----------------------", output_path, append = TRUE)
  } else if (agent_type == "concentration") {
    write_lines("----------------------集中救助エージェントのPLSRモデルにおける予測誤差を算出----------------------", output_path, append = TRUE)
  }
  ...
  # 予測誤差を計算する内部関数
  .calc_error_for_y <- function(y_var, model_info, data_df) {
    
    # モデルの取り出し
    plsr_model <- model_info$model
    
    # 最適成分数の取得
    ncomp_optimal <- model_info$ncomp 
    
    # 最適成分数が0だった場合
    ncomp_to_use <- if (ncomp_optimal == 0) {
      plsr_model$ncomp
    } else {
      ncomp_optimal
    }
    ...
    # 予測値の計算
    pred <- as.numeric(predict(plsr_model, newdata = data_df, ncomp = ncomp_to_use))
  ...
  errors_old_list <- purrr::map(response_variable_names, ~{
    model_info <- plsr_models$old_models[[.x]]
    .calc_error_for_y(.x, model_info, model_data_old)
  })
  ...
  errors_new_list <- purrr::map(response_variable_names, ~{
    model_info <- plsr_models$new_models[[.x]]
    .calc_error_for_y(.x, model_info, model_data_new)
  })
  ...
  errors_all_list <- purrr::map(response_variable_names, ~{
    model_info <- plsr_models$all_models[[.x]]
    .calc_error_for_y(.x, model_info, model_data_all)
  })
  ...
  if (agent_type == "distributed") {
    write_lines("----------------------分散救助エージェントのPLSRモデルにおける予測誤差の計算終了----------------------", output_path, append = TRUE)
    write_lines("", output_path, append = TRUE)
  } else if (agent_type == "concentration") {
    write_lines("----------------------集中救助エージェントのPLSRモデルにおける予測誤差の計算終了----------------------", output_path, append = TRUE)
    write_lines("", output_path, append = TRUE)
  }
