...
#' @param svr_models 作成したSVRモデルのリスト
#'
#' @return 計算した予測誤差

calculate_svr_errors <- function(data_simulation, old_features, new_features, all_features, agent_type, output_path, svr_models) {
  ...
  if (agent_type == "distributed") {
    write_lines("----------------------分散救助エージェントのSVRモデルにおける予測誤差を算出----------------------", output_path, append = TRUE)
  } else if (agent_type == "concentration") {
    write_lines("----------------------集中救助エージェントのSVRモデルにおける予測誤差を算出----------------------", output_path, append = TRUE)
  }
  ...
  # 予測誤差を計算する内部関数
  .calc_error_for_y <- function(y_var, svr_model, data_df) {
    
    # 予測値の計算
    pred <- as.numeric(predict(svr_model,
                               newdata = data_df))
  ...
  errors_old_list <- purrr::map(response_variable_names, ~{
    svr_model <- svr_models$old_models[[.x]]
    .calc_error_for_y(.x, svr_model, model_data_old)
  })
  ...
  errors_new_list <- purrr::map(response_variable_names, ~{
    svr_model <- svr_models$new_models[[.x]]
    .calc_error_for_y(.x, svr_model, model_data_new)
  })
  ...
  errors_all_list <- purrr::map(response_variable_names, ~{
    svr_model <- svr_models$all_models[[.x]]
    .calc_error_for_y(.x, svr_model, model_data_all)
  })
  ...
  if (agent_type == "distributed") {
    write_lines("----------------------分散救助エージェントのSVRモデルにおける予測誤差の計算終了----------------------", output_path, append = TRUE)
    write_lines("", output_path, append = TRUE)
  } else if (agent_type == "concentration") {
    write_lines("----------------------集中救助エージェントのSVRモデルにおける予測誤差の計算終了----------------------", output_path, append = TRUE)
    write_lines("", output_path, append = TRUE)
  }
